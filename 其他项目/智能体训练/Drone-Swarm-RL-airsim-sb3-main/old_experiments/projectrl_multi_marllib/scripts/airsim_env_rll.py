...
class AirSimDroneEnv(MultiAgentEnv):
    def __init__(self, env_config):
        self.image_shape = env_config["image_shape"]
        self.input_mode = env_config["input_mode"]
        self.address = env_config["ip_address"]
        self.num = env_config["num_drones"]
        self.names = ["drone"+str(i) for i in range(0,env_config["num_drones"])]
        self.info = {i: False for i in range(0,env_config["num_drones"])}
        ...
        if self.input_mode == "multi_rgb":
            self.observation_space = gym.spaces.Box(
                low=0, high=255, 
                shape=(env_config["image_shape"][0],env_config["image_shape"][1]*3,1), 
                dtype=np.uint8)
        else:
            self.observation_space = gym.spaces.Box(
                low=0, high=255, shape=env_config["image_shape"], dtype=np.uint8)

        # Defining actions space
        self.action_space = gym.spaces.Box(low=-1.0, high=1.0, shape=(2,), dtype=np.float32) #3.0
    ...
    def step(self, action):
        ...
        reward, done = self.compute_reward(reward, done)
